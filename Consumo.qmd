...
```{python}
metricas = pd.DataFrame(columns=(['Modelo', 'MAPE', 'Tiempo']))
```

```{python}
# Tuner de hiperparametros

def tuner(forecaster_fun, datos_train, datos_test, parametros = ''):
    ...
    if not isinstance(parametros, dict):
        ...
        tiempo = timer_comienzo - timer_final
    ...
    for j in range(0,grilla.shape[0]):
        ...
        if forecaster_fun.__name__ == 'forecast':
            ...
            target_col= 'y', freq= 'M', level=[95], **kwargs)
    ...
    # Devolvemos las predicciones
    return pred, mape_final, tiempo, grilla
...
parametros = {'sp':[6,12, 24]}

pred, mape, tiempo, resultados = tuner(forecaster_fun=ThetaForecaster, parametros=parametros, datos_train=datos_train, datos_test=datos_test)

metricas.loc[len(metricas)] = ['ThetaForecaster', mape, tiempo]
```
...
pred, mape, tiempo, resultados = tuner(forecaster_fun=Prophet, datos_train=datos_train, datos_test=datos_test)

metricas.loc[len(metricas)] = ['Prophet', mape, tiempo]
```
...
pred, mape, tiempo, resultados = tuner(forecaster_fun=AutoARIMA, parametros=parametros, datos_train=datos_train, datos_test=datos_test)

metricas.loc[len(metricas)] = ['AutoARIMA', mape, tiempo]
```

```{python}
# Chronos

parametros = {'model_path': ['amazon/chronos-t5-tiny', 'chronos-bolt-tiny', 'chronos-t5-mini']}
...
pred, mape, tiempo, resultados = tuner(forecaster_fun=PytorchForecastingDeepAR, datos_train=datos_train, datos_test=datos_test)

metricas.loc[len(metricas)] = ['DeepAR', mape, tiempo]
```
...
pred, mape, tiempo, resultados = tuner(forecaster_fun=TimesFMForecaster, parametros= parametros, datos_train=datos_train, datos_test=datos_test)

metricas.loc[len(metricas)] = ['TimesFM', mape, tiempo]
```
...
pred, mape, tiempo, resultados = tuner(forecaster_fun=nixtla_client.forecast, parametros= parametros, datos_train=datos_train, datos_test=datos_test)

metricas.loc[len(metricas)] = ['TimeGPT', mape, tiempo]
...
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = datos['ds'], y= y_pred_forecast, color = 'red', label = 'Forecaster')

plt.subplot(2,1,2)
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = datos['ds'], y= y_pred, color = 'green', label = 'Prophet')
...
plt.subplot(2,1,1)
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = datos['ds'], y= y_pred_arima, color = '#F96E46', label = 'AutoArima')

plt.subplot(2,1,2)
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = datos['ds'], y= y_pred_chronos, color = '#594236', label = 'Chronos')
```
...
plt.subplot(2,1,1)
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = time_gpt1['ds'], y= time_gpt1['TimeGPT'], color = '#820263', label = 'TimeGPT1')

plt.subplot(2,1,2)
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = time_gpt2['ds'], y= time_gpt2['TimeGPT'], color = '#60993E',label = 'TimeGPT2')
```
...
plt.subplot(2,1,2)
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = datos['ds'], y= y_pred_lstm, color = '#04A777', label = 'LSTM')
```
...
plt.subplot(2,1,1)
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = datos_test['ds'], y= y_pred_xgb, color = '#F96E46', label = 'XGBoost')

plt.subplot(2,1,2)
plt.plot(datos['ds'], datos['y'])
sns.lineplot(x = datos_test['ds'], y= y_pred_lgb, color = '#594236', label = 'LighGBM')
```
