...
![Red neuronal completamente conectada](../Imgs/red_conectada.png){#fig-redneuro width=80%}
...
![Red neuronal completamente conectada](../Imgs/conexiones_redneuro.png){#fig-conexiones width=80%}
...
![Verificación del contexto](../Imgs/attention_matrix.png)
  ...
  Ahora que se conoce que tokens son relevantes para otros tokens, es necesario saber como son afectados. Para esto existe otra matriz de valores ($W_V$) que es post-multiplicada por cada vector de entrada resultando en los vectores de valor ($\vec V$) que son multiplicados a cada columna. La suma por filas devuelven el vector ($\Delta \vec E$) que debe ser sumado al vector de entrada original de cada token.
...
4. Suma y normalizado: En lugar de simplemente pasar los datos por las capas, las conexiones residuales se añaden sobre el vector de entrada en la salida de cada capa. Esto es lo que se hizo cuando se sumaron los cambios al vector de entrada, en lugar de directamente modificar el vector.
...
Donde $|\nu_{ts}|$ es el tamaño del diccionario de tokens, el cúal depende del número de intervalos creados. $z_{n+h+1}$ es la serie transformada en *tokens*, de la cual las primeras $n$ observaciones se usarán como entrenamiento para pronosticar las siguientes $h$, y se agrega al final un token $\texttt{EOS}$ que se utiliza comúnmente en los modelos de lenguaje para denotar el final de la secuencia. $\mathcal{p}_\theta$ es la probabilidad estimada por el modelo bajo la parametrización $\theta$.
...
Para ARIMA se usará el método de Box-Jenkins. En este método se compararán aquellos modelos que cumplan con los supuestos, y se elegirá como mejor combinación de parámetros aquella que minimize el AIC (*Akaike Information Criterion*), medida de ajuste que penaliza por la cantidad de parámetros.
...
- Personas registradas con empleo asalariado en el sector educativo privado de Argentina mensuales.
...
problemas de salud priorizados*. Municipalidad de Rosario.]. La serie mostró una marcada estacionalidad, con picos en los meses de invierno, y una fuerte caída en 2020, atribuida a las medidas sanitarias adoptadas durante la pandemia de COVID-19. Esta estacionalidad es más discernible en el gráfico @fig-atenciones_estacionalidad del anexo.
...
La cantidad de personas en Argentina con empleo asalariado en el área de enseñanza y registradas en el sector privado aumenta casi constantemente año tras año. Presenta descensos drásticos en los meses de diciembre y enero. En esta serie también se evidencian las consecuencias de la pandemia. Los datos fueron relevados del informe de Situación y evolución del Trabajo Registrado^[**Secretaría de Trabajo, Empleo y Seguridad Social**. (2025). *Situación y evolución del Trabajo Registrado*. [https://www.argentina.gob.ar/trabajo/estadisticas/situacion-y-evolucion-del-trabajo-registrado](https://www.argentina.gob.ar/trabajo/estadisticas/situacion-y-evolucion-del-trabajo-registrado)].
...
Se debe aclarar que tanto la humedad como la presión atmosférica se suponen conocidas en las horas a pronosticar. Si bien esto no tiene sentido en una aplicación real, existen ocasiones en el que las variables exógenas son conocidas a futuro, y se utiliza esta serie meramente con el objetivo de probar la explotación de este tipo de información por parte de los distintos modelos. En esta tesina ciertos de los modelos presentados tienen la característica de poder usar la información de las variables exógenas sin necesidad de conocer sus valores futuros, sin embargo para garantizar que los resultados sean comparables se asumirá que los valores de humedad y presión atmosférica son conocidos a futuro para todos los modelos. 
...
En la serie de atenciones se puede ver que tanto en las autocorrelaciones como en las autocorrelaciones parciales, hay valores significativos en el cuarto y doceavo rezago, lo que podrían significar que existen componentes MA o AR en la parte estacional o estacionaria de la serie.
...
Para la serie de atenciones se seguirá trabajando con el segundo modelo propuesto y con el seleccionado de manera automática, ya que el parámetro MA del primer modelo prupuesto no es significativo, esto se puede comprobar en la salida del modelo en el [anexo 8.2](#salidas_arima). Por otro lado, los dos modelos probados para la serie de trabajadores seguirán siendo utilizados. El modelo automático para la serie de temperaturas no goza de estacionariedad en la parte estacional, por lo que se proponen los modelos $SARIMAX(1,1,1)(1,0,1)_{24}$, $SARIMAX(1,1,0)(2,0,1)_{24}$ y $SARIMAX(1,1,0)(1,1,0)_{24}$, de los cuales únicamente el último cumple con todas las propiedades buscadas, con un AIC de 1119.3.

Las salidas de todos los modelos se pueden ver en el [anexo](#salidas_arima) para comprobar los resultados.
...
Con el propósito de no poblar de gráficos el documento, el resto de los gráficos para la comprobación de supuestos de los demás modelos se encuentran en el [anexo 8.3](#supuestos_arima). En estos se destaca que todos los modelos, con excepción del seleccionado automáticamente para los trabajadores, superaron la comprobación de supuestos. El problema de este modelo se puede ver en el gráfico de autocorrelaciones en la figura @fig-residTrabajadoresAuto del anexo, donde se observa la existencia de correlación en los residuos, supuesto que es sumamente importante al ajustar modelos ARIMA. Si bien los residuos del modelo seleccionado manualmente para los trabajadores no cumple con normalidad, visible en el histograma de la figura @fig-residTrabajadores1 del anexo, es sencillo ver que esto se debe a unos poco valores extremos, y que sin estos es muy probable que sí se distribuyan normalmente.
...
metricas_1.loc[0] = ['ARIMA', 3, mape_atenciones_2_3, is_atenciones_2_3, 0]
metricas_1.loc[1] = ['ARIMA', 6, mape_atenciones_2_6, is_atenciones_2_6, 0]
metricas_1.loc[2] = ['ARIMA', 12, mape_atenciones_2, is_atenciones_2, 0]

metricas_2.loc[len(metricas_2)] = np.NAN
metricas_2.loc[len(metricas_2)] = np.NAN
metricas_2 = metricas_2.shift().shift()
metricas_2.loc[0] = ['ARIMA', 3, mape_trabajadores_1_3, is_trabajadores_1_3, 0]
metricas_2.loc[1] = ['ARIMA', 6, mape_trabajadores_1_6, is_trabajadores_1_6, 0]
metricas_2.loc[2] = ['ARIMA', 12, mape_trabajadores_1, is_trabajadores_1, 0]

metricas_3.loc[len(metricas_3)] = np.NAN
metricas_3.loc[len(metricas_3)] = np.NAN
metricas_3 = metricas_3.shift().shift()
metricas_3.loc[0] = ['ARIMA', 6, mape_temperatura_3_6, is_temperatura_3_6, 0]
metricas_3.loc[1] = ['ARIMA', 12, mape_temperatura_3_12, is_temperatura_3_12, 0]
metricas_3.loc[2] = ['ARIMA', 24, mape_temperatura_3, is_temperatura_3, 0]
```
...
Un problema que se encuentra es que el intervalo de confianza para las atenciones por guardia toma valores negativos, algo claramente ilógico. Este problema no es único de ARIMA y se verá reflejado en los pronósticos de los demás modelos. Esto se puede resolver ajustando modelos sobre el logaritmo de la variable, y luego transformando los valores pronosticados a la escala original. Esta solución queda propuesta para una futura extención.
...
Los pronósticos con XGBoost fueron muy buenos sobre las 3 series. Si algo se puede remarcar es que en la serie de atenciones el pronóstico puntual no capta del todo el pico de atenciones en invierno, sin embargo sí está contemplado por el intervalo de confianza del 80%. Otro aspecto interesante es que los intervalos para la serie de temperaturas son bastante estrechos y en ocasiones no logra cubrir los valores reales.
...
Las redes neuronales tienen tendencia a sobreajustar, para evitar esto existen numerosas alternativas. En primer lugar se puede optar por frenar el entrenamiento antes de completar todas las iteraciones si es que no se ve mejora en la función de pérdida, esto se conoce como *early stopping*. Otra opción es "ignorar" una cierta proporción de neuronas en cada iteración, que es equivalente a entrenar distintas redes neuronales. Esta última técnica es denominada *dropout*.

Para cada serie se entrenaron modelos LSTM con 300 iteraciones y una tasa de aprendizaje de 0.001. Se adoptó una paciencia para el *early stopping* de 10, esto quiere decir que si la función de pérdida no muestra mejoras en 10 iteraciones seguidas se deteniene el entrenamiento. Además, se probaron las siguientes características para la estructura del modelo:
...
Los pronósticos sobre todas las series fueron correctos, sin embargo se destaca la falta de ajuste en el pico de atenciones por guardia y un corto período de temperaturas que no son cubiertas por el intervalo del 80%. Se puede apreciar como en la serie de atenciones, que no tenía un patrón estacional muy marcado, ajustaron mejor aquellos modelos con un solo rezago de entrenamiento, mientras que en la serie de trabajadores, la cual contaba con una estacionalidad clara, ajustaron mejor aquellos con 24 rezagos. Además, estas dos series se ven diferenciadas por la proporción de *dropout* con la que cuentan los modelos seleccionados. Estos aspectos tal vez se deban a las diferencias en variabilidad estacional que tienen las series, aspecto que se pudo ver en el gráfico @fig-box. Otra curiosidad es la presencia de redes de una sola capa de 32 neuronas para los pronósticos a largo plazo, y las de doble capa con 24 y 42 neuronas en los pronósticos a medio plazo.
...
Para las comparativas en la primera serie se utiliza el modelo ARIMA que mejor ajustó, el cual fue el segundo modelo ajustado manualmente.
...
![Comparación de métricas de evaluación entre distintos modelos y horizontes para la serie de atenciones.](../Imgs/tabla_resumen_1.png)

El modelo ARIMA superó a los modelos más modernos en términos de MAPE a corto y largo plazo, pero sus *Interval Scores* son mucho más grandes a comparación de otros modelos con MAPEs similares. Ningún modelo se destacó claramente en ningún horizonte. 
...
![Comparación de métricas de evaluación entre distintos modelos y horizontes para la serie de trabajadores.](../Imgs/tabla_resumen_2.png)

Todos los modelos pronosticaron excepcionalmente bien la cantidad de trabajadores en el sector privado de educación. Se destacan sobre todo TimeGPT a corto plazo y LSTM en el largo plazo. 
...
![Comparación de métricas de evaluación entre distintos modelos y horizontes para la serie de temperaturas.](../Imgs/tabla_resumen_3.png)

Para la serie de temperaturas, que contaba con la humedad y la presión atmosférica como variables exógenas, tanto ARIMA como LSTM tuvieron resultados muy superiores al resto de modelos en el corto plazo. Pronosticando medio día hacia adelante, TimeGPT y otra vez LSTM fueron los modelos con mejor rendimiento. Pronosticando un día entero XGBoost superó con creces al resto de modelos.

\newpage

# 5. Conclusiones	

Está claro que ningún modelo es totalmente superior al resto y que no existe combinación perfecta de hiperparámetros que logren el mejor ajuste en cualquier circunstancia. A medida que se avanzó por el documento, las diferencias, ventajas y desventajas de cada método se fueron evidenciando. 

Ajustar un modelo ARIMA es un trabajo completamente manual, tomó su requerido tiempo y aplicarlo a otras series implicaría empezar de cero. En términos de resultados logró muy buenos MAPEs en casi todas las ocasiones, sin embargo la amplitud de los pronósticos probabilísticos llevaron a *Interval Scores* muy altos en comparación a otros modelos.

En los algoritmos de aprendizaje automático se tuvieron que definir una serie de características para ayudar al modelo a ajustarse automáticamente a los datos, si se deseara pronosticar otra serie, sería prudente cambiar las características a otras más acordes a los datos. Comparando con otros modelos no se destacaron especialmente en ninguna ocasión, con excepción de XGBoost en el pronóstico a largo plazo de la serie de temperaturas, sin embargo tuvieron consistentemente buenos resultados en todas las series.

Con LSTM simplemente se tuvo que definir la estructura del modelo. Fue el modelo más exigente computacionalmente, pero a cambio logró muy buenas métricas. Cuenta además con la ventaja de que para ajustarse a otra serie no es necesario ningún cambio significativo en el modelo. 

Tanto con TimeGPT como con Chronos solo fue necesario alimentarles los datos y su periocidad. TimeGPT al igual que LSTM fue de los modelos que mejores pronósticos realizaron. Por otro lado, los ajustes de Chronos fueron de los que peores métricas obtuvieron, tal vez esto se solucionaría eligiendo otro modelo de la familia de Chronos diferente a `bolt-small`, o haciendo uso del ajuste fino. Si bien TimeGPT y Chronos requieren mínimo esfuerzo y conocimiento, también son poco personalizables y si el ajuste no es bueno hay poco que se pueda hacer. En el caso de TimeGPT se suma el problema de no ser de código abierto. No se conoce del todo como es específicamente el ajuste de los modelos, además de poder ser inaccesible de un día para otro sujeto a las decisiones de la empresa. Al comienzo de esta tesina la API de TimeGPT ofrecia un plan gratuito con un número de consultas mensuales limitadas, sin embargo a la fecha de publicación los planes de suscripción cambiaron y este plan ya no está disponible, siendo reemplazado por una prueba gratuita de consultas limitadas para usuarios nuevos únicamente por 30 días. Este cambio afectó mucho el desarrollo del trabajo y se tuvieron que recortar múltiples expansiones a raíz de esto.

En este documento se presentan 3 contribuciones claves para el campo de la estadística. En primer lugar y como objetivo principal de esta tesina, la introducción de los modelos transformadores para el pronóstico de series temporales. Además, se exploró una nueva medida del error para evaluar el desempeño de los modelos, el *Interval Score*, el cúal no mide el ajuste del pronóstico puntual como las métricas que se usan habitualmente, sino que tiene en cuenta el pronóstico probabilístico. El último gran aporte es la construcción de intervalos de pronóstico por medio de *conformal predictions*, que no depende de conocer la distribución de los residuos.
...
Para comparar los modelos también se pudo haber estudiado el tiempo de cómputo en los ajustes. Una prueba de esto fue realizada, pero los métodos utilizados para obtener estos resultados no fueron del todo adecuados y ante la falta de alternativas se decidió recortar estos resultados y no mostrarlos empíricamente, sin embargo se hicieron menciones a los resultados a lo largo del trabajo. A raíz de esto, queda propuesto estudiar distintas metodologías para medir los tiempos de cómputo de los modelos.
