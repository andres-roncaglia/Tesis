...
from Funciones import plot_forecast, load_env, autocorr_plot
...
Una serie que si presenta tendencia y estacionalidad al mismo tiempo es la cantidad de personas con empleo asalariado en el área de enseñanza, registradas en el sector privado. La cantidad de empleados aumenta casi constantemente año tras año, presentadose descensos drásticos en los meses de diciembre y enero. En esta serie también se evidencian la consecuencias de la pandemia. Se buscará pronosticar los trabajadores registrados en el área de enseñanza del año 2024.
...
\multicolumn{1}{|c|}{$SARIMA(0,1,0)(1,1,0)_{12}$} & \multicolumn{1}{l|}{857.6}                & \multicolumn{1}{c|}{Si}              & \multicolumn{1}{c|}{Si}             & \multicolumn{1}{c|}{Si}              & Si             \\ \hline
\multicolumn{1}{|c|}{$SARIMA(0,1,1)(0,1,0)_{12}$} & \multicolumn{1}{l|}{852.1}                & \multicolumn{1}{c|}{Si}              & \multicolumn{1}{c|}{Si}             & \multicolumn{1}{c|}{Si}              & Si             \\ \hline
...
\multicolumn{1}{|c|}{$SARIMA(0,1,1)(1,1,0)_{12}$} & \multicolumn{1}{l|}{366.6}                & \multicolumn{1}{c|}{Si}              & \multicolumn{1}{c|}{Si}             & \multicolumn{1}{c|}{Si}              & Si             \\ \hline
\multicolumn{1}{|c|}{$SARIMA(2,0,0)(2,1,0)_{12}$} & \multicolumn{1}{l|}{364.8}                & \multicolumn{1}{c|}{Si}              & \multicolumn{1}{c|}{Si}             & \multicolumn{1}{c|}{Si}              & Si             \\ \hline
\multicolumn{6}{|c|}{TEMPERATURA}                                                                                                                                                                                                  \\ \hline
\multicolumn{1}{|c|}{$SARIMA(1,1,1)(2,0,1)_{24}$} & \multicolumn{1}{l|}{1067.1}               & \multicolumn{1}{c|}{Si}              & \multicolumn{1}{c|}{Si}             & \multicolumn{1}{c|}{No}              & Si             \\ \hline
\end{tabular}
\end{table}

Para la serie de atenciones se seguirá trabajando con el segundo modelo propuesto y con el seleccionado de manera automática, ya que el parámetro AR del primer modelo prupuesto no es significativo. Por otro lado, los dos modelos probados para la serie de trabajadores seguirán siendo utilizados. El modelo automático para la serie de temperaturas no goza de estacionariedad, por lo que se proponen los modelos $SARIMA(1,1,1)(1,0,1)_{24}$, $SARIMA(1,1,0)(2,0,1)_{24}$ y $SARIMA(1,1,0)(1,1,0)_{24}$, de los cúales únicamente el último cumple con todas las propiedades buscadas, con un AIC de 1119.3.

Lo siguiente es comprobar que los residuos se comportan como ruido blanco.

```{python}
# Pronosticos automaticos

plt.figure(figsize=(9, 5))
plt.subplot(3,1,1)
plot_forecast(data = atenciones_guardia, forecast = resultados_1_arima['pred'], pred_color = 'red', label = 'ARIMA', long = 24)

plt.subplot(3,1,2)
plot_forecast(data = trabajadores, forecast = resultados_2_arima['pred'], pred_color = 'red', label = 'ARIMA', long = 24)

plt.subplot(3,1,3)
plot_forecast(data = tiempo_rosario, forecast = resultados_3_arima['pred'], pred_color = 'red', label = 'ARIMA', long = 36)
```
...
- TO DO: 
      ...
      revisar uso de variables exogenas (deberia usar solo la parte entrenamiento?, entrenamiento y despues brindar para el testeo? o usar todo como conocido?) CAPAZ MEJOR COMO PASADAS
      revisar niveles de confianza en todos los metodos y en el interval score
