...
![Diagrama de la estructura del modelo](../Info%20tesis/Imgs/modelo2.png){#fig-modelo}
...
![(before applying softmax) for example: Fluffy and Blue contribute to creature](../Info%20tesis/Imgs/attention_matrix.png)
...
![](../Info%20tesis/Imgs/attention_matrix_softmax.png)
...
$$
W_l = IS_l - II_l \hspace{1, cm} O_l = \left \{ \matrix{ \frac{2}{\alpha}(Z_n(l) - Z_{n+l}) \hspace{10,mm} \text{si } Z_n(l) > Z_{n+l} \\ 0 \hspace{1,cm} \text{en otro caso} \hfill}\right. \hspace{1, cm} U_l = \left \{ \matrix{ \frac{2}{\alpha}(Z_{n+l} - Z_n(l)) \hspace{10,mm} \text{si } Z_n(l) < Z_{n+l} \\ 0 \hspace{1,cm} \text{en otro caso} \hfill}\right.
$$
...
- Mejorar LSTM
- Conseguir intervalos de confianza para chronos y XGB y LGBM

- Dividir la funcion de hiperparametros en mas funciones
...
Arima: hacer
Prophet: listo (hiperparametros?)
XGB: Buscar intervalos (bootstrap?)
LGBM; hacer
LSTM: Listo (hiperparametros)
TimeGPT: hiperparametros, corregir graficos
Chronos: hacer
