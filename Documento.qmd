...
Sea $e_l = Z_{n+l} - \hat Z_n(l)$ el error de la l-ésima predicción, donde $\hat Z_n(l)$ es el pronóstico $l$ pasos hacia adelante, algunas medidas del error son:

- Error Cuadrático Medio (*Mean Square Error (MSE)*):

$$
MSE = \frac{1}{M}\sum^M_{l=1}e_l^2
$$

- Error absoluto medio (*Mean Absolute Error (MAE)*):

$$
MAE = \frac{1}{M}\sum^M_{l=1}|e_l|
$$

- Porcentaje del error absoluto medio (*Mean Absolute Percentage Error (MAPE)*)

$$
MAPE = (\frac{1}{M}\sum^M_{l=1}|\frac{e_l}{Z_{n+l}}|)\cdot 100 \%
$$
...
- tsfresh para la extraccion de caracteristicas
- sktime engloba muchos modelos para series de tiempo
- Agregar deepAR
- Conseguir los intervalos de confianza
- ¿podria hacer una medida del error que tenga en cuenta los intervalos de confianza y no simplemente la media de la prediccion?
