...
- Red Neuronal (neuronal newtwork): Las redes neuronales son un tipo de programa o modelo de *machine learning* que toma decisiones de la misma forma que las personas, usando procesos que simulan la forma biologica en la que trabajan la neuronas para indentificar fenómenos, evaluar opciones y llegar a conclusiones. **EXTENDER CON BREVE EXPLICACION SOBRE CAPAS Y TIPOS COMO CNN Y RNN**

- Aprendizaje profundo (*Deep learning*): Es una subdivisión del *machine learning* que usa redes neuronales más complejas para acercarse aún más a la forma en la que el cerebro humano trabaja. 

Los modelos transformadores (*transformer models*) son un tipo de modelos con arquitectura de aprendizaje profundo (*deep learning*). Su auge se ve explicado por su alta eficiencia para entrenar y realizar inferencias en comparación con otros métodos de aprendizaje profundo tales como las redes neuronales recurrentes (RNN) o redes neuronales convolucionales (CNN). Esta eficiencia se debe al uso de mecanismos de atención, presentado en la publicación '[*Attention is all you need*](https://arxiv.org/pdf/1706.03762)' de Google, este mecanismo se detallará más adelante para poder ser seguido con ejemplos.
...
TimeGPT es un modelo de tipo transformer con mecanismos de autoatención (del inglés *self-attention mechanisms*) que **no** es de código abierto. La autoatención captura dependencias y relaciónes en la secuencias de valores que se alimentan al modelo, logrando poner en contexto a cada observación. De acuerdo con los creadores de TimeGPT, 
...
![Diagrama de la estructura del modelo](../Info%20tesis/Imgs/modelo.png){#fig-modelo}
...
2. Codificación posicional: Típicamente se introduce como un set de valores adicionales o vectores que son añadidos los vectores de entrada antes de alimentarlos al modelo ($\vec E \Leftarrow \vec E + \vec P$). Estas codificaciones posicionales tienen patrones específicos que agregan la información posicional del token.
...
Sea $e_l = Z_{n+1} - \hat Z_n(l)$ el error de la l-ésima predicción, donde $\hat Z_n(l)$ es el pronóstico $l$ pasos hacia adelante, algunas medidas del error son:

- Error Cuadrático Medio (*Mean Square Error (MSE)*):

$$
MSE = \frac{1}{M}\sum^M_{i=1}e_i^2
$$

- Error absoluto medio (*Mean Absolute Error (MAE)*):

$$
MAE = \frac{1}{M}\sum^M_{i=1}|e_i|
$$

- Porcentaje del error absoluto medio (*Mean Absolute Percentage Error (MAPE)*)

$$
MAPE = (\frac{1}{M}\sum^M_{i=1}|\frac{e_i}{Z_{n+l}}|)\cdot 100 \%
$$

Aquel modelo que tenga el menor valor en estas medidas será el mejor.
