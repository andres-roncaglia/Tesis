...
def autocorr_plot(vector, lags, atype = 'acf'):
    ...
    autocorrelaciones = pd.DataFrame({
        ...
        'acf' : acf(vector, nlags=lags),
        'pacf' : pacf(vector, nlags=lags, method = 'ywm') 
    })
    ...
    col = np.where(autocorrelaciones[atype] < lower_bound,'green', np.where(autocorrelaciones[atype] > upper_bound, 'green', 'b'))
...
resid_manual_est = (resid_manual - np.mean(resid_manual))/ np.std(resid_manual)
```
...
# Histograma
plt.subplot(2,2,1)
sea.histplot(resid_manual_est, bins=30)

# Serie de los residuos
plt.subplot(2,2,2)
sea.lineplot(x = empleo['empleados'], y = resid_manual_est)
plt.axhline(y = 3, color = 'black', ls = '--')
plt.axhline(y = -3, color = 'black', ls = '--')

# Autocorrelaciones
plt.subplot(2,2,3)
autocorr_plot(resid_manual_est, lags=30)

plt.subplot(2,2,4)
autocorr_plot(resid_manual_est, lags=30, atype='pacf')
```
